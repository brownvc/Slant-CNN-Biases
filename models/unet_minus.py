...
class UNet_minus(nn.Module):
    def __init__(self, channels=1, latent_dim=128):
        super(UNet_minus, self).__init__()
        ...
        self.deconv2d2 = self.deconv2d(chann_in=self.gf * 4, chann_out=self.gf * 4)
        self.deconv2d3 = self.deconv2d(chann_in=self.gf * 4, chann_out=self.gf * 2)
        self.deconv2d4 = self.deconv2d(chann_in=self.gf * 2, chann_out=self.gf)
        self.final = self.final_layer()

    def conv2d(self, chann_in, chann_out, k_size=(4, 4)):
        layer = nn.Sequential(
            nn.Conv2d(chann_in, chann_out, kernel_size=k_size, stride=2, padding=1),
    ...
    def deconv2d(self, chann_in, chann_out, k_size=(4, 4)):
        ...
        layer = nn.Sequential(
            ...
            nn.Conv2d(chann_in, chann_out, kernel_size=k_size, stride=1, padding='same'),
            nn.BatchNorm2d(chann_out),
            nn.ReLU(),
        )
        return layer
    ...
    def final_layer(self):
        layer = nn.Sequential(
            nn.UpsamplingNearest2d(scale_factor=2),
            nn.Conv2d(self.gf, self.channels, kernel_size=(4, 4), stride=1, padding='same'),
            nn.Tanh(),
        )
        return layer
    ...
    def forward(self, input):
        ...
        u4 = self.deconv2d2(u3)
        u5 = self.deconv2d3(u4)
        u6 = self.deconv2d4(u5)
        out = self.final(u6)
    ...
    #     d5 = MaxPooling2D((4, 4), padding='same')(d4)
    #     d6 = Flatten()(d5)
    #     d7 = Dense(self.latent_dim)(d6)
    #
    #     # Decoder for reflectance map
    #     u1_1 = Dense(256)(d7)
    #     u1_1 = Reshape((8, 8, 4))(u1_1)
    #     u1_2 = deconv2d(u1_1, d4, self.gf * 4)
    #     u1_3 = deconv2d(u1_2, d3, self.gf * 4)
    #     u1_4 = deconv2d(u1_3, d2, self.gf * 2)
    #     u1_5 = deconv2d(u1_4, d1, self.gf * 2)
    #
    #     u1_6 = UpSampling2D(size=2)(u1_5)
    #     output = Conv2D(self.channels, kernel_size=4, strides=1, padding='same', activation='tanh')(u1_6)
    #
    #     return Model(d0, [output, d7])
